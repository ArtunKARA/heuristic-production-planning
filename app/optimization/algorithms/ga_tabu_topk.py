...
def planned_iterations(payload: Dict[str, Any]) -> int:
    ...
    top_k = max(0, int(payload.get("top_k", 0)))
    return 1 + max_iter + (max_iter if (tabu_iter > 0 and top_k > 0) else 0)
...
def run(ctx: AlgorithmContext) -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    top_k = max(0, int(payload.get("top_k", 3)))
    top_k = min(top_k, population_size)
    tabu_iter = max(0, int(payload.get("tabu_iter", 3)))

    rng = _ga_create_rng(mutation_cfg, salt=977)
    ...
    for generation_no in range(1, max_iter + 1):
        ...
        if tabu_iter > 0 and top_k > 0:
            ranked = sorted(population, key=lambda item: _rank_key(item[1]))[:top_k]
            best_tabu_state, best_tabu_eval = ranked[0]
            for cand_state, cand_eval in ranked:
                improved_state, improved_eval = _tabu_search(
                    ...
                    label_prefix="tabu-topk",
                    record_step_labels=False,
                    summary_label=None,
                )
                if is_better(improved_eval, best_tabu_eval):
                    best_tabu_state, best_tabu_eval = improved_state, improved_eval
                if is_better(improved_eval, cand_eval):
                    population = _inject_candidate(population, (improved_state, improved_eval), population_size)
            ctx.record(best_tabu_state, best_tabu_eval, f"tabu-topk-{generation_no}")
...
ALGO = Algorithm(
    spec=AlgorithmSpec(
        code="ga_tabu_topk",
        name="GA+Tabu (Top-K)",
        desc="Her GA jenerasyonu sonunda Top-K adaylara kisa tabu iyilestirmesi uygulanir.",
        params={
